...
Polymer({
  ...
  behaviors: [FirebaseStorageBehavior],
  ...
  observers: ["__filesChanged(files, auto)"],
   ...
   * @override
   */
  upload: async function (files, path) {
    if (!this.app) {
      this.fire("error", new Error("No app configured!"));
    }
    const uploads = await this._putMultipleFirebaseFiles(
      path || this.path,
  ...
  _putMultipleFirebaseFiles: async function (path, files) {
    this._log(
      "Putting Multiple Firebase files at",
    ...
    if (files && files.length > 0) {
      ...
      for (var i = 0; i < files.length; i++) {
        var uploadTask = await this.__put(
          path,
          files[i],
          this.metadata ? this.metadata : files[i].metadata
        );
        uploads.push(uploadTask);
        this.push.bind(this, "uploadTasks", uploadTask);
      }
      return uploads;
    }
  },
